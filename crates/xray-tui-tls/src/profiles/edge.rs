//! Microsoft Edge 130 fingerprint profile.
//!
//! Edge (Chromium-based) is nearly identical to Chrome but omits
//! `compress_certificate` while keeping ALPS. Ported VERBATIM from
//! `thirdparty/tls-fingerprint/src/profiles/edge.rs`.

use crate::spec::grease::GREASE_PLACEHOLDER;
use crate::spec::{ClientHelloSpec, ExtensionSpec, KeyShareGroup, SessionIdSpec};

/// Cipher suites used by Edge 130 (in order).
...
/// Signature algorithms used by Edge 130.
const SIG_ALGOS: &[u16] = &[
...
/// The Edge 130 `ClientHello` spec — no `compress_certificate`, ALPS for h2.
#[must_use]
pub fn spec() -> ClientHelloSpec {
    ClientHelloSpec {
        ...
        extensions: vec![
            ...
            // Edge does not include compress_certificate unlike Chrome.
