//! Chrome 119 fingerprint profile.
//!
//! Chrome 105–119 era — before ALPS (0x4469) and `compress_certificate`
//! (0x001B) became part of the standard Chrome `ClientHello`. Cipher suites
//! are identical to Chrome 130 but the extension list is shorter, giving a
//! distinct JA3 hash. Ported VERBATIM from
//! `thirdparty/tls-fingerprint/src/profiles/chrome119.rs`.

use crate::spec::grease::GREASE_PLACEHOLDER;
use crate::spec::{ClientHelloSpec, ExtensionSpec, KeyShareGroup, SessionIdSpec};

/// Cipher suites used by Chrome 119 (in order).
...
/// Signature algorithms used by Chrome 119.
const SIG_ALGOS: &[u16] = &[
...
/// The Chrome 119 `ClientHello` spec — no ALPS, no `compress_certificate`.
#[must_use]
