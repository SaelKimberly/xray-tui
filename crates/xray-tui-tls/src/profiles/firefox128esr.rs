//! Firefox 128 ESR fingerprint profile.
//!
//! Firefox 128 ESR (Jun 2024) — the main difference from Firefox 121 is
//! that `compress_certificate` now advertises both brotli (0x0003) AND zlib
//! (0x0002). Ported VERBATIM from
//! `thirdparty/tls-fingerprint/src/profiles/firefox128esr.rs`.

use crate::spec::{ClientHelloSpec, ExtensionSpec, KeyShareGroup, SessionIdSpec};

/// Cipher suites used by Firefox 128 ESR (in order).
...
/// Firefox 128 ESR signature algorithms.
const SIG_ALGOS: &[u16] = &[
...
/// The Firefox 128 ESR `ClientHello` spec — no GREASE, X25519-only key
/// share, brotli + zlib `compress_certificate`.
#[must_use]
pub fn spec() -> ClientHelloSpec {
    ClientHelloSpec {
        ...
        extensions: vec![
            ...
            // FF128 ESR: both brotli (0x0003) and zlib (0x0002).
            ExtensionSpec::CompressCertificate(vec![0x0003, 0x0002]),
