//! Opera 114 fingerprint profile.
//!
//! Opera is Chromium-based and uses GREASE, but differs from Chrome 130 by:
//! - No ALPS (0x4469)
//! - No `compress_certificate` (0x001B)
//! - No `signed_certificate_timestamp` (0x0012)
//! - Slightly different signature algorithm list (no `rsa_pkcs1_sha384`)
//!
//! These differences give Opera a distinct JA3 hash from Chrome.
//! Ported VERBATIM from `thirdparty/tls-fingerprint/src/profiles/opera114.rs`.

use crate::spec::grease::GREASE_PLACEHOLDER;
use crate::spec::{ClientHelloSpec, ExtensionSpec, KeyShareGroup, SessionIdSpec};

/// Cipher suites used by Opera 114 (in order).
...
/// Signature algorithms used by Opera 114 (no `rsa_pkcs1_sha384`).
const SIG_ALGOS: &[u16] = &[
...
/// The Opera 114 `ClientHello` spec — no ALPS, no `compress_certificate`, no SCT.
#[must_use]
pub fn spec() -> ClientHelloSpec {
    ClientHelloSpec {
        ...
        extensions: vec![
            ...
            // No signed_certificate_timestamp (Opera omits SCT).
            ...
            // No compress_certificate, no ALPS.
