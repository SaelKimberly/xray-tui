...
//!
//! This module currently owns the *provisioner contract* (a fingerprint-shaped
//! `ClientHello` the reality connector later fills). The connector itself
//! lands in M3; `FixedChrome133`'s body is filled by the reality ring port
//! (Task 13).
...
pub trait HelloProvisioner: Send + Sync {
    ...
    fn provision(&self, params: &HelloProvisionParams) -> Result<ProvisionedHello>;
}
...
impl HelloProvisioner for FixedChrome133 {
    // Implemented in M3 with the reality ring port; M2 = unreachable stub.
    fn provision(&self, _params: &HelloProvisionParams) -> Result<ProvisionedHello> {
