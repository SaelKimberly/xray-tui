...
use crate::{
    ...
    matchers::{CompiledDomain, CidrSet, CidrSetBuilder, DomainRulesSpec},
};
...
impl Engine {
        ...
        for (idx, rule) in rs.rules.into_iter().enumerate() {
            let items = match rule.cond {
                ...
                Cond::Any(_) | Cond::Invert(_) => {
                    return Err(RouteError::Unsupported("Cond::Any/Invert not evaluated yet"));
                }
            };
            rules.push(CompiledRule { name: rule.name, items, action: rule.action });
        }
        Ok(Self {
            rules,
    ...
    pub fn set_event_sink(
        &mut self,
        tx: tokio::sync::mpsc::UnboundedSender<RouteEvent>,
    ) {
...
fn compile_item(item: crate::ir::MatchItem, idx: usize) -> Result<ItemMatcher, RouteError> {
    ...
    Ok(match item {
        M::Domain { exact, suffix, keywords, regexes } => ItemMatcher::Domain(
...
mod tests {
    ...
    const ALL_PORTS: PortRange = PortRange { start: 0, end: u16::MAX };

    fn base_default() -> RuleSet {
        RuleSet {
            rules: vec![],
            default: DefaultRoute::Route { tag: "direct".to_owned() },
    ...
    fn single_item_rule(item: MatchItem, action: Action) -> Rule {
        Rule { name: None, cond: Cond::All(vec![item]), action }
    }

    fn rs(rules: Vec<(MatchItem, Action)>) -> RuleSet {
        let mut set = base_default();
        set.rules = rules.into_iter().map(|(item, action)| single_item_rule(item, action)).collect();
        set
    }

    fn meta(host: &str, port: u16, network: NetworkMask) -> ConnMeta {
        ConnMeta {
            target: NetAddr { host: NetHost::new(host), port },
            network,
            inbound_tag: None,
            source: None,
    ...
    fn route(tag: &str) -> Action {
        Action::Route { tag: tag.to_owned(), override_addr: None }
    }
    ...
    fn first_match_wins_in_declaration_order() {
        let e = Engine::build(rs(vec![
            (MatchItem::Ports(vec![PortRange { start: 80, end: 80 }]), route("a")),
            (MatchItem::Network(NetworkMask::TCP), route("b")),
        ]))
        .unwrap();
        assert_eq!(
            e.decide(&mut meta("example.com", 80, NetworkMask::TCP)),
            Decision::Route { tag: "a".to_owned(), override_addr: None }
        );
    }
    ...
    fn unmatched_items_skip_the_rule_not_abort() {
        ...
        assert_eq!(
            ...
            Decision::Route { tag: "direct".to_owned(), override_addr: None }
        );
    }

    #[test]
    fn default_fallback_when_nothing_matched() {
        let e = Engine::build(rs(vec![(MatchItem::Ports(vec![
            PortRange { start: 9999, end: 9999 },
        ]), route("rare"))]))
        .unwrap();
        assert_eq!(
    ...
    fn reject_and_hijackdns_are_terminal() {
        let reject = Engine::build(rs(vec![(MatchItem::Ports(vec![ALL_PORTS]), Action::Reject {
            method: RejectMethod::Drop,
        })]))
        .unwrap();
        assert_eq!(
            reject.decide(&mut meta("example.com", 443, NetworkMask::TCP)),
            Decision::Reject { method: RejectMethod::Drop }
        );

        let hijack =
            Engine::build(rs(vec![(MatchItem::Ports(vec![ALL_PORTS]), Action::HijackDns)]))
                .unwrap();
        assert_eq!(hijack.decide(&mut meta("dns.example.com", 53, NetworkMask::UDP)), Decision::HijackDns);

        // DefaultRoute maps identically.
        let mut set = base_default();
        set.default = DefaultRoute::Reject { method: RejectMethod::DefaultReply };
        let default_reject = Engine::build(set).unwrap();
        assert_eq!(
            default_reject.decide(&mut meta("example.com", 1234, NetworkMask::TCP)),
            Decision::Reject { method: RejectMethod::DefaultReply }
        );
    }
    ...
    fn event_sink_receives_decision_applied_after_terminal_decision() {
        ...
        let mut rules = rs(vec![(domain(&["block.example.net"], &[]), route("blackhole"))]);
        ...
        assert_eq!(
            d,
            Decision::Route { tag: "blackhole".to_owned(), override_addr: None },
            "decision itself is unchanged by the sink"
        );

        let ev = rx.try_recv().expect("event emitted after terminal decision");
        let crate::events::RouteEvent::DecisionApplied { rule_name, tag, sni, at } = ev else {
    ...
    fn no_sink_means_no_event_and_decisions_still_flow() {
        ...
        assert!(e.decide(&mut meta("h.example", 1, NetworkMask::UDP)).is_routed_to("a"));
    }

    #[test]
    fn domain_exact_and_suffix_match_destination_only_when_domain_target() {
        let e = Engine::build(rs(vec![(domain(&["example.com"], &["sub.corp"]), route("d"))]))
            .unwrap();
        assert!(e.decide(&mut meta("example.com", 80, NetworkMask::TCP)).is_routed_to("d"));
        assert!(e.decide(&mut meta("x.sub.CORP", 80, NetworkMask::TCP)).is_routed_to("d"));
        // Suffix requires a label boundary.
        assert!(!e.decide(&mut meta("xfoo.com", 80, NetworkMask::TCP)).is_routed_to("d"));
        // IP targets fail domain items this task (Task 12 wires enrichment).
        assert!(!e.decide(&mut meta("93.184.216.34", 80, NetworkMask::TCP)).is_routed_to("d"));
    }
    ...
    fn regexes_participate_in_domain_matching() {
        ...
        assert!(e.decide(&mut meta("img.cdn7.net", 443, NetworkMask::TCP)).is_routed_to("fast"));
        assert!(!e.decide(&mut meta("plain.net", 443, NetworkMask::TCP)).is_routed_to("fast"));
    }
    ...
    fn ip_cidr_hits_via_literal_target_or_resolved_host_ips() {
        ...
        let e = Engine::build(rs(vec![(
            MatchItem::IpCidr { cidrs: cidrs(), private: false, geo_country: vec![] },
            route("lan"),
        )]))
        .unwrap();
        // Literal IP target.
        assert!(e.decide(&mut meta("10.1.2.3", 8000, NetworkMask::TCP)).is_routed_to("lan"));
        assert!(!e.decide(&mut meta("8.8.8.8", 8000, NetworkMask::TCP)).is_routed_to("lan"));
    ...
    fn private_cidr_matches_private_ranges_and_geo_country_is_inert() {
        let e = Engine::build(rs(vec![(
            MatchItem::IpCidr { cidrs: vec![], private: true, geo_country: vec![] },
            route("home"),
        )]))
        .unwrap();
    ...
    fn source_items_need_source_data_and_use_both_source_fields() {
        let sp = |p: u16| {
            MatchItem::SourcePorts(vec![PortRange { start: p, end: p }])
        };
        let e = Engine::build(rs(vec![(sp(5000), route("elevated"))])).unwrap();
        // Absent source => item false.
        assert!(!e.decide(&mut meta("srv.example", 443, NetworkMask::TCP)).is_routed_to("elevated"));
        ...
        via_resolved.source_resolved_ips.push(IpAddr::from([10u8, 77, 0, 1]));
    ...
    fn network_mask_requires_connection_within_item_bits() {
        ...
        .unwrap();
        assert!(both.decide(&mut meta("h", 1, NetworkMask::TCP)).is_routed_to("any"));
        assert!(both.decide(&mut meta("h", 1, NetworkMask::UDP)).is_routed_to("any"));

        // Single-bit item needs equality: conn carrying both bits escapes it.
        let tcp_only = Engine::build(rs(vec![
            (MatchItem::Network(NetworkMask::TCP), route("tcp")),
        ]))
        .unwrap();
        let mut both_bits = meta("h", 1, NetworkMask { tcp: true, udp: true });
        assert!(!tcp_only.decide(&mut both_bits).is_routed_to("tcp"));
    }

    #[test]
    fn protocol_requires_sniffed_agreement_and_none_misses() {
        let e = Engine::build(rs(vec![(MatchItem::Protocol(crate::ir::SniffedProtocol::Tls), route(
            "tls-out",
        ))]))
        .unwrap();
        assert!(!e.decide(&mut meta("a.b", 443, NetworkMask::TCP)).is_routed_to("tls-out"));
    ...
    fn inbound_tag_membership_matches_outbound_tag_inert_until_field_exists() {
        let ib = MatchItem::InboundTag { tags: vec!["proxy-in".to_owned()] };
        ...
        let ob = MatchItem::OutboundTag { tags: vec!["x".to_owned()] };
        let e2 = Engine::build(rs(vec![(ob, route("out"))])).unwrap();
        assert!(!e2.decide(&mut meta("h", 1, NetworkMask::TCP)).is_routed_to("out"));
    }
    ...
    fn build_validates_compilable_pieces() {
        ...
        let res = Engine::build(rs(vec![(
            MatchItem::IpCidr {
                cidrs: vec![Cidr { addr: IpAddr::from([10u8, 0, 0, 0]), bits: 33 }],
                private: false,
                geo_country: vec![],
            },
            route("x"),
        )]));
        assert!(res.is_err(), "out-of-range prefix length must fail at build time");

        // Inverted port range sanity check.
        let res = Engine::build(rs(vec![(
            MatchItem::Ports(vec![PortRange { start: 200, end: 100 }]),
            route("x"),
        )]));
        ...
        assert!(matches!(Engine::build(any_set), Err(RouteError::Unsupported(_))));
    }
