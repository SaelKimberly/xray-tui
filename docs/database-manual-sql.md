...
| `endpoint_ip.rs` `replace` | Delete-then-insert the whole set, deduped by key (typed delete + typed create per row — the mix that is fine because the set is 1–3 rows and TTL-gated) | — | Not a raw-SQL site: kept typed on purpose, and it is the model for "small set, typed is fine" | `resolving_a_dns_host_moves_its_stored_key` |
