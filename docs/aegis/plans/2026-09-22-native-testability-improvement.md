...
**And a large budget does NOT meet it — corrected after nearly acting on the opposite reading.** `real_ping_timeout_secs` is unbounded and user-settable, so a 20 s budget is reachable, and that briefly looked like the trigger. It is not, because T4's clamp is `min(step_constant, remaining)`: it caps a step DOWN and never lets one exceed its constant, so a 20 s budget still dies at the 10 s `DIAL` cap. The constant binding there is the design, not a trap. What T4 changes is only that the constants become *derived* rather than independent — no observable difference at any budget — so the honest reading of the trigger is *"when the per-step caps should stop being the binding constraint"*, which nothing has asked for. The 39-site change would have bought that and nothing else.
