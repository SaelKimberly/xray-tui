...
**DECIDED 2026-09-22: T4 stays DEFERRED — terminal, by user decision.** Asked directly ("should a user-set budget override the engine's per-step caps?"), the answer was to leave T4 deferred. So the spec's `min(cap, remaining)` stands, no site is touched, and the 44-site hygiene change is not made. The reasoning above is kept because it is *why* the question was put rather than acted on; it is no longer an open item.

**The delivery claim, had it been taken, would have been "every enumerated site classified"** — kept for a future un-deferral. `step_limit`'s arithmetic is the easy half; the hard half is that a *forgotten* site is invisible — it compiles, and at a 5 s budget it behaves identically to a clamped one. So the exemption tests prove two sites are NOT clamped and cannot prove none was missed. The completeness evidence would have to be the enumerated site list itself — **58 `timeouts::*` sites, file:line → clamped / exempt-with-reason** — diffable against the module tree (`grep -rn "timeouts::" crates/xray-tui-native/src`).
...
**Disposition: item 2 RETURNS TO DESIGN, and that is TERMINAL — by user decision (2026-09-22).** The spec's §10 recovery clause (*"the mlkem differential says the client is at fault: item 2 returns to design; it is not quietly closed"*) fired, and asked directly whether to start the design cycle or accept the return-to-design as terminal, the answer was to accept it. So T9's second half (the fix) is **unfinished by decision**: the localization is the deliverable, the pq-enc row stays ignored, and the gate stays closed. Nothing further is attempted.
