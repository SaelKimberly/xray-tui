...
- **Changed, user-visible**: the fingerprint capability verdict for 1,165 links (2026-09-21 audit snapshot), and the class a failure is counted under.
- **Changed, NOT user-visible — and the budget's VALUE does not move either**: T4 (deferred) makes the engine's step limits clamps against the caller's attempt deadline, and the shipped `real_ping_timeout_secs` default **stays 5 s** (M0b: basis 2's p99 is 3,251 ms, so 5 s already clears it; no success-based p99 exists to justify a raise). An earlier version of this line claimed the default itself changed, which was false while T4 was deferred.
- **A correction to T4's own rationale, recorded because it nearly produced a 44-site change on a false premise.** T4's clamp is `min(step_constant, remaining)` (§5.2), so it caps a step DOWN and never lets one exceed its constant. A budget of 20 s therefore still dies at the 10 s `DIAL` cap — the constant binds, and that is the design, not the trap. So "the clamp removes a trap for users who raise the timeout above 10 s" is **wrong**, and it is not a reason to un-defer: what T4 changes is only that the constants become *derived* rather than independent, with no observable difference at any budget. The un-defer trigger stays as stated (a budget value >10 s) and the honest reading of it is *"when the per-step caps should stop being the binding constraint"*, which nothing has asked for.
...
**And a large budget does NOT meet it — corrected after nearly acting on the opposite reading.** `real_ping_timeout_secs` is unbounded and user-settable, so a 20 s budget is reachable, and that briefly looked like the trigger. It is not, because T4's clamp is `min(step_constant, remaining)`: it caps a step DOWN and never lets one exceed its constant, so a 20 s budget still dies at the 10 s `DIAL` cap. The constant binding there is the design, not a trap. What T4 changes is only that the constants become *derived* rather than independent — no observable difference at any budget — so the honest reading of the trigger is *"when the per-step caps should stop being the binding constraint"*, which nothing has asked for. The 44-site change would have bought that and nothing else.

**Decision needed from the user, not resolvable by acting.** Reconciling the two readings: **the trigger asks whether the clamps change behaviour, and under `min(cap, remaining)` they never do at any budget** — a step is capped DOWN, and `probe.rs`'s wrapper already bounds the whole attempt. So what is overstated is §5.2's own rationale ("one number, the budget is authoritative"), not the trigger.

The residual, stated precisely: **for a budget above 10 s the caps still bind, and under `min` that is per-step POLICY, not a defect.** The only way T4 would change it is by switching the **semantics** from `min(cap, remaining)` to `remaining` — budget wins per step, and the caps become dead on the probe path. That is a *different design*:

- it contradicts the `min` the spec pinned (§5.2), so it is a spec revision, not an implementation choice;
- it removes the per-step caps that stop a dead server from eating the whole budget — with `remaining`, a 20 s budget lets one dial spend 20 s.

So it is a **decision to put to the user**: *should a user-set budget override the engine's per-step caps?* Until that is answered, T4 stays deferred and no site is touched.

**The delivery claim would be "every enumerated site classified", not "the clamp works".** `step_limit`'s arithmetic is the easy half; the hard half is that a *forgotten* site is invisible — it compiles, and at a 5 s budget it behaves identically to a clamped one. So the exemption tests prove two sites are NOT clamped and cannot prove none was missed. The completeness evidence must therefore be the enumerated site list itself — **58 `timeouts::*` sites, file:line → clamped / exempt-with-reason** — recorded where a reviewer can diff it against the module tree (`grep -rn "timeouts::" crates/xray-tui-native/src`).

**Scope if un-deferred** (measured, so it is not re-discovered): **58 `timeouts::*` sites across 23 files**, of which **~14 are deliberate exemptions** — `inbound/outbound.rs` (7), `inbound/mod.rs` (5) and `inbound/http.rs` (2), the LOCAL server's own handshakes, which are not attempts. The ~44 remaining are on the attempt path (`context.rs`, `security/mod.rs`, `transport/*`, `protocol/*`, `probe.rs`). The budget is carried on `LinkContext` — the per-link policy surface, already in scope at every engine site — rather than threaded by signature, which is what keeps it from being a 40-signature change. Two exemptions a reviewer must confirm by name: `inbound/*`, and `outbound.rs::relay`'s `TUNNEL_READ` (*"bounds inactivity of the TUNNEL, not of one direction"*).
