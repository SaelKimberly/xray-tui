...
**Status: DEFERRED with T4 — this design is not landed.** The observable contract it exists for is already met and tested by `probe.rs`'s outer wrapper; the plan's T4 carries the reasoning, the un-defer trigger and the measured 58-site scope. **And this section's `min` is itself an open decision**: switching the semantics to `remaining` (budget wins per step, caps dead on the probe path) would change behaviour for budgets above 10 s — but it removes the per-step caps that stop a dead server from eating the whole budget, so it is a spec revision the plan puts to the user rather than an implementation choice.
