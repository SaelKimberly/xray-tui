...
**Status: DEFERRED with T4 — this design is not landed, and that is terminal by user decision (2026-09-22).** The observable contract it exists for is already met and tested by `probe.rs`'s outer wrapper; the plan's T4 carries the reasoning and the measured 58-site scope. **This section's `min` is settled, not open**: asked whether a user-set budget should override the engine's per-step caps — the semantics change that would make it `remaining`, with the caps dead on the probe path — the answer was to keep T4 deferred, so `min(cap, remaining)` stands and a budget above 10 s keeps meeting the per-step caps by design.
...
- **The release binary's origin is outside this repository — that much is now closed, the identity is not.** Every launch I made used the **debug** binary (the `tui-test` harness, and `/tmp/tui_visual/run.sh` which execs `target/debug/xray-tui`), and **the tree spawns no `xray-tui` at all**: grepped `crates/**/tests`, `crates/**/src`, the `justfile`, `.cargo/config.toml` and every non-`target/` file, then read the two candidates the grep left. `crates/xray-tui-native/tests/probe_e2e.rs` resolves and spawns **xray-core** (with an `Xray` banner check) and is `#[ignore]`d; `crates/xray-tui/tests/e2e_real_feed.rs` is the TUI crate's only test file, is `#[ignore]`d and uses an in-memory DB. So the spawner is not a test, not a recipe and not a config in this workspace — the remaining candidate is a long-lived tool outside it (the codegraph MCP server is the only one observed holding this workspace open).
- Its argv was `target/release/xray-tui xray-tui` — a relative path with its own name as an argument, which no human invocation produces — so a programmatic spawn is likelier than a manual one. That is still a likelihood, not a finding, and the identity stays open.
...
- **The writer is NOT the quality gate.** Checked after a full `just quality-gate code` run: the live DB's mtime was unchanged (`18:35:37`) and no `-wal` sidecar appeared. So whatever wrote it is **outside the gate** — a tool or a manual start — which narrows the field without naming it. No `xray-tui` process is alive now either (the only `ps` hit is the codegraph MCP server).
...
**T2's re-run, after T8 (verified 2026-09-22).** The plan's T2 asked for a forced override and a connect; the reachable form of that is the **build**, which is what the connect path calls. `build_xray_refuses_the_removed_http_transport` drives `ConfigBuilder::build` with a `vless` config carrying an `Http` transport and `CoreType::Xray`: it now returns **`BuildError::Support`** naming the transport — where before it produced a config the core refused to load — and the SAME config still builds for sing-box, so the refusal is scoped to the core that removed the transport. That closes T2's *"after T8, a named downgrade"* clause at the level that matters (no downgrade code exists by design: decision 20 already routes native-capable kinds to native, and an explicit xray override is answered by a warning, not by overriding the user back).
