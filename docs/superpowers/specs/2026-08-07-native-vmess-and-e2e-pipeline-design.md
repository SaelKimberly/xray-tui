...
- `vmess::connect` (in `protocol/mod.rs`, replacing `not_impl`): write the sealed request (bounded by `timeouts::PROTOCOL`) and return `VmessClientStream` **without waiting for the response header**. The stream peels the AEAD response header on the first read (the proven VLESS strategy — xray vs sing-box send-timing differs; a client that blocks reading the header before sending payload can deadlock against a lazy writer). Response records follow the header. `ConnectShape` already routes VMess to `TcpStream`.
