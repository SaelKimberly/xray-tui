...
**Schema management**: no migration machinery. `db.push_schema()` runs only when the `PRAGMA user_version` tag differs from `SCHEMA_VERSION = 13`; toasty emits `CREATE TABLE` without `IF NOT EXISTS`, so on an existing DB the push fails and `Database::open` **deletes the file** and recreates the schema — a tag bump is a data reset, never a migration (7 = per-kind binary identity re-key, 8 = `endpoint_rank`, 9 = the durable-facts pass: task columns / `cred_hash` / `parent_id` dropped, timestamps to epoch seconds, `last_seen_at` index, 10 = `endpoint_ip`, the address set as a packed-key table replacing the JSON-array `resolved_as` column, 11 = `endpoint_ip.country`, 12 = `profile_stats.purge_reason`, 13 = no column change at all: the ws `?ed=` query is hoisted into the identity-hashed `max_early_data`, so the bump exists only to drop the data rather than carry the re-key's duplicate set). System groups created by `init_default_groups()`. Known quirk: toasty's `push_schema` leaves a cross-process SQLITE_BUSY write lock on the db file for the life of the process (external sqlite3 access blocked while the app runs; app's own single-pooled-connection ops unaffected).
...
**DB query monitoring**: `crates/xray-tui/src/ops/db_monitor.rs` — an in-process `DbMonitorLayer` (a second `tracing::Layer` on the same registry) aggregates toasty 0.10's built-in per-statement `toasty::query` events (`duration_ms`/`rows`/`error`/`db.statement`; DEBUG, WARN past `slow_statement_threshold`) with the `retry_on_busy` retry count, keyed by `Database` method name. Two-source: `on_event` folds query metrics via the enclosing `#[instrument(target = "db_method")]` span, `on_close` folds the method's final `retries`. Registry is a bounded in-memory per-method `Registry` (circular sample window, p50/p99, fail/retry rollup, slowest-statement sample); ranked dump on the `d` key on the Logs tab. `main.rs` `LogVisitor` also captures the `toasty::query` fields so a slow-query panel line reads `slow query [312.4ms]: <sql>` rather than the bare message.
