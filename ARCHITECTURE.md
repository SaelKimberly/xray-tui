...
- `profiles.rs` — 17-column endpoint rows (tree marker, indicator, #, Type, `[flag address:port][feat]=>{config}=>[ 12 ][outbound country]`) over a sortable DataTable. The Test column (`[ 12 ]` etc.) shows the active protocol's delay colored by threshold (green <500ms, yellow ≥500ms, red ≥1000ms) or red problem labels `[name]` (DNS unresolved), `[fast]`/`[real]` (every protocol of the endpoint failed that type — from session-only `AppState.ping_status` rounds); it sits after the `}=>` arrow, before Outbound's `[` bracket. Expanding an endpoint renders a rounded panel in the row's own height (IPs line + 10-col per-protocol sub-table: marker, hex id, last seen, last used, config type, delay, speed, traffic, outbound, country). Sub-table rows sorted by test priority (real-ok > fast/udp-ok > untested > real-err > fast-err > DNS-unresolved; latency only within success tiers)); the selected sub-row renders as a REVERSE highlight (`ThemeStyles::panel_row_selected`: fg foreground + `bg Color::Reset` + bold — drops the panel's `surface` highlight back to the common background; explicit Reset bg required because ratatui `Cell::set_style` merges). Height-aware scroll offset (`compute_scroll_offset`) keeps expanded rows reachable; `panel_w` is viewport-capped so narrow terminals don't panic. Footer/overlays use `host:port` (remarks r…
...
which updates the ProfileExtension (delay, delay_source, ip_info) in memory and persists via upsert_profile_extension()
(error events mutate nothing — the write is gated on `error.is_none()`), then re-sorts the owning endpoint's
protocols by test priority (TcpPing/RealPing results; `selected_sub` remapped by protocol id) and invalidates
the main-table sort cache.
