...
- ✅ Write path — the flush drains to ONE coalesced patch per link and `apply_link_patches` wrote each existing row with a single literal `UPDATE` over the patch's groups (no per-row SELECT, no bound parameters; the existence probe was one statement per 400-row chunk). *(Superseded by Phase 24: the existence probe and the per-row `UPDATE` are retired in favour of one multi-row `INSERT … ON CONFLICT` per chunk — the per-row form was one statement per link, because turso has no `UPDATE … FROM (VALUES …)` — measured 29.0 ms → 10.0 ms per 512-patch window.)*
