...
- ✅ Write path — the flush drains to ONE coalesced patch per link and `apply_link_patches` writes each existing row with a single literal `UPDATE` over the patch's groups (no per-row SELECT, no bound parameters; the existence probe is one statement per 400-row chunk). Turso has no `UPDATE ... FROM (VALUES ...)`, so per-row statements stay per-row — measured via a probe rather than assumed.
