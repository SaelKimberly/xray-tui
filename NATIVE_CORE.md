...
|------|------|-----------|----------|
| 1 — offline | `cargo test -p xray-tui-tls -p xray-tui-native --features native-e2e --lib`; `cargo nextest run -p xray-tui-tls -E 'test(every_roster_hello_has_rfc_minimal_extension_bodies)'` | unit: wire encodings, RFC 8448 key-schedule vectors, GREASE pairing, JA3/JA4 goldens, VMess Go byte-vectors, rustls-server interop (dev-dep), multi-record reassembly, TLS 1.2 client path (X25519 + secp256r1 ECDHE, AEAD key block, explicit-nonce records, DOWNGRD-sentinel + RFC 7627 EMS guards), Spider-X fallback, transport framing (httpupgrade header set, xhttp chunk/seq/pacing + padding, v2rayhttp method/authority), vision codec (padded frames, TLS filter, Direct splice) + hermetic fake-vision-server, vless UDP (packet framing, packetaddr codec, PacketConn, hermetic fake-UDP-server), v1.mux.cool mux (frame codec, `MuxClient` multiplexer, hermetic fake-mux-server), vless XUDP (mux UDP sessions — per-packet dests + `GlobalID`, `PacketConn` `XUdp` mode, hermetic fake-mux UDP session), trojan UDP (address-framed `PacketConn` over one reused frame buffer, domain-reply → no-address, length-exact payload read, split-flight reassembly, bad-CRLF/unknown-ATYP rejection), vmess UDP (record-boundary datagram view over the AEAD stream — one record per datagram, cancel-safe `write_datagram`, xray 8192-byte chunk cap, header-dest mismatch refusal), hy2 UDP (UDPMessage wire layout, borrowed-view serializer, defrag reassembly — out-of-order, duplicate and stale-packet discard, drop-undeliverable-and-keep-reading, bandwidth-string parser), shadowsocks (method table + both password KDFs, classic-AEAD chunk framing against independent goldens, 2022 header layout/counter/`request_salt`, both UDP datagram codecs + session table + replay windows), xhttp h3 (the `decideHTTPVersion` dispatch rule — single `h3` ALPN → QUIC, `http/1.1` → h1, 0/2+ or other → h2; the shared v3 protocol over the `V3Send` seam; hermetic h3 server double over loopback QUIC; ML-KEM-768 primitives (liboqs roundtrips + size pins), TLS hybrid curves (X25519MLKEM768 key-share encode/parse, `pq || classical` key schedule, fake-PQ-server handshake), VLESS `mlkem768x25519plus` encryption (parser, relay/AEAD chain, xor-mode masking, hermetic double), REALITY 4588 hybrid share (hermetic fake REALITY PQ server)) | 870 lib tests (210 tls + 660 native; native incl. 65 shadowsocks tests (9 method-table/KDF, 13 classic-AEAD chunk framing + independent goldens + cancel-state, 13 2022 header layout/counter/`request_salt`/single-write, 4 family dispatch, 26 UDP datagram codecs + session table + replay windows) + 36 vision-module tests (incl. 2 hermetic fake-vision-server) + 24 vless UDP/XUDP-path tests (udp framing, packetaddr codec, PacketConn incl. `XUdp` mode, 2 hermetic fake-UDP-server) + 42 mux tests (v1.mux.cool codec + `MuxClient`/`SessionStream`/`UdpSession` + 3 hermetic fake-mux-server incl. the fake-mux UDP session) + 68 mKCP tests (segment codec, KCP session — RTO/RTT + send/recv windows + retransmit + state machine, hermetic fake-peer over loopback UDP) + 39 non-vless UDP-path tests (19 trojan, 8 vmess udp, 12 hy2 udp))
| 2 — live grader | `cargo run -p xray-tui-tls --example grader -- --profile <id>`; `cargo run -p xray-tui-tls --example grader -- --roster [--family <name>] [--sample]`; `cargo test -p xray-tui-tls --test tls_peet_ws -- --ignored`; `cargo run -p xray-tui-tls --example probe_host -- <host> <port> <sni> [attempts] [alpn,list] [--verify]` | ClientHello graded against tls.peet.ws; `probe_host` dials a real `host:port` with the engine's plain-TLS path and reports per-attempt outcomes (the tool that classifies a recorded handshake failure as server-side or engine-side: the 2026-09-15 batch's `record too large` rows handshook 60/60 here — transient path — while a TLS-1.2 P-256-only host failed 3/3 until secp256r1 ECDHE landed; the 2026-09-16 `no certificate` rows reproduced 3/3 with `--verify` and were fixed by the RFC 8879 arm). `probe_host` is `insecure` by DEFAULT, which short-circuits the verifier before the chain is looked at — re-probing a verify-class failure (`server presented no certificate`, `chain verification failed`, `server name mismatch`, `invalid leaf`) therefore REQUIRES `--verify`, else the probe reports OK and misclassifies the row as transient | Chrome130 JA4 `t13d1516h2_8daaf6152771_f37e75b10bcc`; Firefox 128 resolves next-modern to the kept `firefox_139_windows_desktop` (JA3 `fdb1b23bd019c5596f46c8bf59f21968` + JA4 `t13d1516h2_8daaf6152771_02713d6af862`); kept 71-profile roster live sweep (71/71 both runs) + offline JA4 gate + Cloudflare amiabot report (see `docs/tls-fingerprint-roster.md`, `docs/amiabot-roster-report.md`) |
| 3 — real-core e2e | `XRAY_TUI_CORE_BIN_DIR=<dir> cargo test -p xray-tui-native --features native-e2e --test vless --test vmess --test trojan --test hysteria2 --test shadowsocks` | native client against spawned xray-core (26.3.27) + sing-box (1.13.16) servers, transport + TLS-variant matrix **including one row per generated-tier fingerprint preset (`fp("firefox")`/`fp("safari")`/`fp("ios")`, both cores — the fingerprint axis is otherwise `chrome`, i.e. the hand-declared `chrome_130`; REALITY rows provision `FixedChrome133`, so no generated-tier hello is exercised there)**, transport + TLS-variant matrix, VLESS vision flow axis, VLESS UDP datagram path, VLESS mux axis, VLESS XUDP axis, VLESS mKCP axis, VLESS XHTTP/3 axis, VLESS ML-KEM PQ axis, VMess/Trojan/Hysteria2/Shadowsocks UDP datagram rows, Shadowsocks classic-AEAD + 2022-blake3 TCP matrix + TLS-fingerprint row | 176 tests = 170 green + 6 documented ignored (vless 78+6, vmess 53, trojan 14, hysteria2 3, shadowsocks 22; ignored: the 4 ws/grpc plain-into-reality-server semantic rows × both cores + reality-pq + pq-enc (SP7 — see the ML-KEM axis below); single-core rows run only on the serving core: xhttp + xhttp-h3 + kcp + pq-enc + the shadowsocks TLS row on xray, v2rayhttp + ws/httpupgrade-reality + mux-vision + vision-udp443 on sing-box)
...
**What tier 1's roster gate cannot see** (and what the 2026-09-17 run exposed):
`tests/generated_ja4_gate.rs` validates the built hello against this crate's
own parser and the JA4 extension-id/cipher/sig-alg projection, and tier 2's
grader validates it against tls.peet.ws — a Go peer that ignores an extension
it does not implement. Neither can see an extension **body** the RFC forbids, so
two shipped: `encrypted_client_hello` with a 0-byte body (36 generated rows) and
`compress_certificate` with an empty algorithm list (28 rows). Cloudflare and
Google answer `alert 2 50 (decode_error)` to both, which made every
firefox/safari/ios preset unprobeable and unconnectable there. The gates that
close it: the RFC-minimum body sweep in tier 1
(`profiles::tests::every_roster_hello_has_rfc_minimal_extension_bodies`) and, in
tier 2, `cargo test -p xray-tui-tls --test strict_peer_presets -- --ignored`,
which handshakes a Cloudflare front with every preset `profile_for` resolves.
...
**Shadowsocks axis** (`tests/shadowsocks.rs`): 11 cases × {xray 26.3.27, sing-box 1.13.16} =
22 entries, 21 of them real dials (the TLS row is xray-only, below). Classic 2017 AEAD:
...
| UDP | dial-end carrier for both families (`protocol/ss/udp.rs`, `PacketTunnel::Ss`): `connect_udp` binds its own `UdpSocket` to the server's port, so a datagram carries no in-tunnel framing and `chain.rs::ss_udp_guard` refuses any chain around it (mirrors `quic_guard`) — as does a non-empty `security` on the UDP path. Classic = fresh salt+subkey per datagram, address always on the wire; 2022 = AES methods seal the body under the plaintext separate header's `[4..16]` nonce, `ChaCha` prepends a 24-byte nonce and seals `[client_session_id ‖ packet_id ‖ body]` with the PSK. Session table = one current + one previous server session (spec §3.2.4) with a 1024-id replay window anchored at the highest committed id, a ±30 s timestamp gate and a 60 s rotation quiet period. e2e 4 rows: `udp_aead`, `udp_2022`, `udp_2022_aes128`, `udp_2022_chacha20` × both cores. The relay needs the SERVER to listen on UDP — xray only with `network: "tcp,udp"` (default is tcp-only, and xray's own *client* instead tunnels UDP inside its TCP stream), sing-box by default — so a default xray inbound leaves this row without a UDP leg |
| Status | Native client complete + e2e (`tests/shadowsocks.rs`): 11 cases × both cores = 22 entries (21 dials — the TLS row is xray-only) — the three classic ciphers, the three 2022 methods (incl. the 16-byte-PSK BLAKE3 XOF truncation), cert TLS + chrome fingerprint, four UDP relay rows (the 2022 `ChaCha` merged-id shape included). Deferred: SIP003 plugins, the legacy stream ciphers (sing-box), 2022 multi-user EIH. |
