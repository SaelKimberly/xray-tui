...
- `crates/xray-tui/src/lib.rs` — AppState (incl. profile_gen), Tab, SortColumn, ProfileRow, LogLine, SettingsMode (Split variant), SettingsSection (incl. SpeedTest), SplitFocus, SplitRightPane, CoreEvent (incl. TestTypeUpdate, HostFeaturesLoaded, EndpointInfoUpdated), EndpointInfo, testing_details map, endpoint_info map, format_ts, iso_to_flag
