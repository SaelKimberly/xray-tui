...
- `crates/xray-tui-native/src/inbound/` — local SOCKS5 server (`Socks5Inbound`, RFC 1928 + RFC 1929 auth): accept loop → `xray-tui-route` `Engine` (`decide_async`) → tagged `Outbound` (Direct / Block / Proxy — the proxy reusing `crate::connect`). TCP CONNECT only; BIND/UDP-ASSOCIATE refused `0x07`, `HijackDns` refused `0x02`.
