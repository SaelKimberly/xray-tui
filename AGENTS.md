...
- `crates/xray-tui-tls/src/` — ring-based TLS client engine (TLS 1.3, plus a TLS 1.2 ECDHE+AEAD client path reached on a 1.2 ServerHello) with browser fingerprint mimicry + REALITY (workspace crypto standard: ring). Module layout: `spec/` (declarative `ClientHelloSpec`/`ExtensionSpec`/`SessionIdSpec` model + exact RFC 6066/8446 wire encoding, GREASE), `profiles/` (two-tier roster: `hand_selected.rs` `spec!`-declared wire-exact `chrome_130`/`edge_106` + `generated/` 69-entry JA4-faithful roster — the `select_roster` kept subset of the 1825-entry ja4db manifest; resolution is next-modern with cross-triple os-drop), `client/` (unified engine entry: `TlsConfig { mode, server_name, alpn, rng }`, `TlsMode::{Plain, Reality}`, one `connect` — plain + REALITY share the `handshake::drive` driver), `hello/` (ClientHello builder + parser), `crypto/` (key schedule, AEAD, X25519 + JA3/JA4 codecs; `mlkem.rs` ML-KEM-768 via liboqs/`oqs` vendored — SP7 PQ), `record/` (record layer + `TlsStream` AsyncRead/AsyncWrite), `handshake/` (TLS 1.3 client handshake + TLS 1.2 fallback driver on a 1.2 ServerHello, HRR detection, `ServerVerifier` seam; X25519MLKEM768 hybrid key exchange — pq‖classical IKM per the Go wire order), `verify/` (`WebPkiVerifier`: chain+SAN+`CertificateVerify` signature via rustls-webpki; `insecure`/`pin_sha256` trust modes), `reality/` (`HelloProvisioner` + `ProfileProvisioner(BrowserProfile)` + 9-step wire contract, Spider-X h2 fallback; client keypair on x25519-dalek — ring's `EphemeralPrivateKey` is single-use and can't do REALITY's dual agreement), `http2/` (minimal h2 layer: tls.peet.ws grader + Spider-X fallback GETs). Tier-2 verification: `examples/grader.rs` + ignored `tests/tls_peet_ws.rs`. Purpose, profiles, and verification tiers: `NATIVE_CORE.md`.
- `crates/xray-tui-native/src/` — in-process native proxy core (subprocess-free alternative to spawning xray-core/sing-box). Layering (Xray composition order): dial → security → transport upgrade → protocol → tunnel, folded by `connect_chain` — TLS is OUTERMOST, ws/grpc framing runs INSIDE the engine TLS session (matches xray/sing-box; xhttp with a single `h3` ALPN is the exception — `connect_quic` replaces dial + security + upgrade, quinn/rustls is internal and the engine TLS never wraps QUIC). **LinkContext is the per-link policy decision surface** (`context.rs`): `is_tls()` (one TLS/REALITY predicate), `server_name()` (unified SNI — plain `TlsOpts.sni` or the REALITY steal target `RealityOpts.sni`, else endpoint host), `alpn_tokens()`/`alpn_vec()` (canonical `split_alpn`, shared with xhttp's `http_version` decision), `curve_ids()` — every phase reads its policy from the context, never re-derives it. **XHTTP mode auto-rule mirrors xray: `""`/`auto` → packet-up; reality present → stream-one (legacy v1, single full-duplex request, no session id); reality + download_settings → stream-up.** Transports: `transport/tcp.rs` (dial), `transport/ws.rs` (tokio-tungstenite over the engine stream, v2ray Host/path/headers, write-through), `transport/grpc.rs` (h2 over the engine stream, gun mode, `Hunk` protobuf + 5-byte gRPC prefix, deferred response headers via …
...
- `crates/xray-tui-native/src/inbound/` — local SOCKS5 server (`Socks5Inbound`, RFC 1928 + RFC 1929 auth): accept loop → `xray-tui-route` `Engine` (`decide_async`) → tagged `Outbound` (Direct / Block / Proxy — the proxy reusing `crate::connect`). TCP CONNECT plus **UDP ASSOCIATE** (`Socks5InboundConfig::udp`, default on): one relay task per association routes EVERY datagram separately (`NetworkMask::UDP`, whole-payload sniffing gated on `Engine::needs_sniff`), so one association may reach direct and proxy outbounds at once. Direct sends use per-family upstream sockets (both bound on first direct datagram; domain destinations resolve in a spawned task through the association DNS cache). Proxy sends go to a `run_proxy_leg` task that owns the tunnel, splits it (`PacketTunnel::split`) and reads replies in its own task — `PacketTunnel::recv` must never be raced in a `select!`. The association pins to the control connection's peer, expires after `UDP_PIN_DEADLINE` if no datagram arrives, and ends on control-TCP EOF. BIND refused `0x07`; `HijackDns` drops the datagram (TCP: `0x02`). Per-datagram failures drop the datagram, never the association.
...
17. **TLS fingerprint engine** (`crates/xray-tui-tls`, replaces the former "M2" forward reference): browser mimicry is declarative — a `ClientHelloSpec` describes the hello at the semantic level, `profiles/` supplies the two-tier roster (2 wire-exact hand profiles + 69 JA4-faithful generated entries; chrome_130 verified live against tls.peet.ws), and `hello/` encodes it to the exact RFC 6066/8446 wire format. Certificate verification is real WebPKI (`verify::WebPkiVerifier` on rustls-webpki: chain + SAN + `CertificateVerify` signature over the RFC 8446 §4.4.3 transcript; `insecure`/`pin_sha256` trust modes only — never a blanket skip). The REALITY client reuses the same hello machinery (`reality::HelloProvisioner` injects keyshare/SNI/random/sealed SessionId) and builds its dual-agreement X25519 keypair on xray-tui-tls's `crypto::X25519KeyPair` (x25519-dalek runtime dep — ring's `EphemeralPrivateKey` is single-use and cannot do both the REALITY auth ECDH and the TLS keyshare ECDH). `xray-tui-native` security consumes it engine-only: `security::wrap` builds an engine `TlsConfig` and runs `xray_tui_tls::client::connect` (plain + REALITY, Spider-X fallback) — the rustls client path is gone (rustls = server-side test double + the h3 arm's internal quinn TLS).
18. **Transport composition (xray order, hyper-framed upgrades)**: transports are upgrade steps INSIDE the (outermost) security session — ws (tokio-tungstenite), grpc (h2 crate), httpupgrade (RFC 7230 101), xhttp/splithttp and v2rayhttp (hyper) all frame over the engine TLS stream, matching xray/sing-box. **xhttp + a single `h3` ALPN is the exception**: the QUIC dial (`connect_quic`, quinn + h3) replaces the dial + security + upgrade chain entirely (`is_self_contained`) — quinn's rustls is internal, the engine TLS never wraps QUIC (SP5). HTTP/1.1 + HTTP/2 framing is hyper 1.11 on minimal features (`client`+`http1`+`http2`; default features are empty anyway) + hyper-util 0.1.20 `tokio` (`TokioIo`/`TokioExecutor` bridge — hyper 1.x has no tokio blanket impls; hand-rolling would own unsafe cursor code) + http-body-util 0.1.5 `channel` (upload pipes: stream-up POST, v2rayhttp PUT). Rationale: maintainability over hand-rolled HTTP — hyper owns requests/responses/chunked/101; the native layer keeps ownership of the byte stream, the dial, and the timeouts (every hyper read/write wrapped in `tokio::time::timeout`). grpc.rs deliberately stays on the h2 crate (working, Hunk-codec-verified — same h2 0.4 major as hyper's, no duplicate).
19. **Secret wiping (`zeroize`)**: two layers, and the manifest layer is the load-bearing one. (a) **Dependency features** — `zeroize` is NOT in the default feature set of `aes`/`aes-gcm`/`chacha20poly1305`/`ctr`/`hmac`/`sha2`/`md-5`/`blake2` (xray-tui-native) and IS in `x25519-dalek`'s defaults, which `default-features = false` silently strips. Both manifests now list `zeroize` explicitly, so cipher key schedules, hasher blocks, and the REALITY scalar/`SharedSecret` wipe themselves; **re-check `cargo tree -p <crate> -e features | grep zeroize` after any crypto dependency edit** — dropping the feature is invisible at compile time. (b) **Our own buffers** — `Zeroizing<T>` on every owned key buffer: the whole TLS 1.3 key schedule (`hkdf_extract`/`hkdf_expand_label`/`derive_secret`/traffic secrets via `crypto::TrafficSecrets`/`finished_key`), the TLS 1.2 PRF chain (`prf`/`p_hash`/`master_secret*`/`key_block`), `X25519KeyPair::agree`, `reality::auth::derive_auth_key`, the hybrid `pq‖classical` IKM, VMess (`cmd_key`, `chacha20_key_32`, the KDF's heap `msg` buffers, `Session` via `#[derive(ZeroizeOnDrop)]`), VLESS `mlkem768x25519plus` (`nfs_key`, `united`), the trojan hex-SHA224 auth token, `Salamander.psk`, and the SOCKS5 RFC 1929 credential frame. `mlkem::SecretKey`/`SharedSecret` derive `ZeroizeOnDrop` with hand-written length-only `Debug` (the derive would dump keys into a log line); liboqs never wipes its own buffers, so `encapsulate`/`decapsulate` take ownership via `into_vec()` into a `Zeroizing`. **Deliberately not wiped**, with reasons: ring's `LessSafeKey`/`Prk`/`hmac::Tag` are opaque (no wipe hook — the wipeable surface is the derivation buffers on either side); AEAD write IVs are useless without the key inside ring; wire-visible values (nonces, salts, `finished_mac`/`verify_data`, transcript hashes, public keys, `pin_sha256`, REALITY `short_id`); QUIC Initial keys in `xray-tui-route::sniff` (derived from a public salt + DCID — any observer derives them); `xray-tui-proto` config structs and `Socks5InboundConfig.auth` (the same bytes sit plaintext in SQLite and in the generated core config JSON, and they live until process exit, so a wrapper buys nothing for a large ripple). `Zeroizing<[u8; N]>` does NOT coerce to `&[u8]` — pass `.as_slice()`; `Zeroizing<Vec<u8>>` does.
...
- `zeroize` (workspace, `derive` feature) for wiping secret key material — see decision 19; the `zeroize` FEATURE must also stay listed on every crypto dep that touches keys (`aes`, `aes-gcm`, `chacha20poly1305`, `ctr`, `hmac`, `sha2`, `md-5`, `blake2`, `x25519-dalek`), since none of them enable it by default
