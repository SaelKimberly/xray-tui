...
- `crates/xray-tui-db/src/retry.rs` — `retry_on_busy`/`is_busy_error`: retry-with-backoff (20ms doubling, 1.28s cap) for SQLite write contention (`is_serialization_failure` or "database is locked"); wired into `update_endpoint_resolution` + `batch_flush_ping_buffer` so the enrichment herd never drops writes
