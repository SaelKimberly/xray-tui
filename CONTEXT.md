...
| **ProtocolId / uid** | Content hash identifying a protocol config (legacy name: `sub_uid`). `uid = sig ^ cred_hash`, computed by the per-kind binary identity writer in the proto crate (`ProtoIdentity::write_identity`), over non-default fields only, with endpoints (host/port) excluded — one config on many servers dedups to one `Protocol` row. 64-bit, never zero. |
...
- `crates/xray-tui-proto/src/proto_spec/mod.rs` — ProtocolConfig enum, sealed `ProtoIdentity`/`ProtoSpec` traits, dispatch macro; `identity.rs` holds the per-kind binary identity writer (`IdentityWriter`, framed fields, dual sig/cred streams)
...
| Profile identity | Auto-increment PK | `uid = sig ^ cred_hash` content hash (dedup key `Protocols.id`); deterministic for URL-imported and form-created rows alike |
